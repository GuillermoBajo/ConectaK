...
tablero_columna_valida(uint8_t columna) {
	return columna < NUM_COLUMNAS;
}

// la fila es valida?
__inline static uint8_t
tablero_fila_valida(uint8_t fila) {
  return fila < NUM_FILAS;
}

// el color es valido? color=3 es el color de prueba
__inline static uint8_t
tablero_color_valido(uint8_t color) {
